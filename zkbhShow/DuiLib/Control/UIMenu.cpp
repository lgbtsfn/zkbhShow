...
namespace DuiLib {
	...
	CMenuWnd* CMenuWnd::CreateMenu(CMenuElementUI* pOwner, STRINGorID xml, POINT point, CPaintManagerUI* pMainPaintManager, CStdStringPtrMap* pMenuCheckInfo /*= NULL*/, DWORD dwAlignment /*= eMenuAlignment_Left | eMenuAlignment_Top*/)
	{
		...
		pMenu->Init(NULL,pOwner, xml, point, pMainPaintManager, pMenuCheckInfo, dwAlignment);
		return pMenu;
	}
	...
	void CMenuWnd::Init(CControlUI* pcOwner,CMenuElementUI* pOwner, STRINGorID xml, POINT point,
	...
	void CMenuElementUI::CreateMenuWnd()
	{
		...
		m_pWindow->Init(NULL,static_cast<CMenuElementUI*>(this), _T(""), CDuiPoint(), NULL);
	}
